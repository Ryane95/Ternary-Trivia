...
int answerQuestion(team teams[], board jepordy[5][5]) {
  ...
  string prompts[4] = {jepordy[row][col].answer, jepordy[row][col].fake1,
                       jepordy[row][col].fake2, jepordy[row][col].fake3};
  ...
  if (prompts[multipleChoice[choice - 1]] == jepordy[row][col].answer) {
    cout << "The answer is correct. " << jepordy[row][col].points
         << " points.\n";
    teams[turn].points += jepordy[row][col].points;
  } else {
    cout << "The answer is Incorrect. " << jepordy[row][col].points
         << " points.\n";
    teams[turn].points -= jepordy[row][col].points;
  }
  return 0;
...
void restartGame(team teams[], board jepordy[5][5]) {
  ...
  cout << "Would you like to populate your own questions, or get a random "
          "category and question (y/n)? ";
  while ((cin >> request) && request != "y" && request != "Y" &&
         request != "n" && request != "N") {
