 ...
 * "value (1-5) indicating value/points/difficulty" "Question" "answer1" ...
...
vector<string>
splitLine(string line) // split thee eline by the tab that seperare the info
{
  vector<string> parts;
  string temp = "";
...
void readFile(string filename) // read in the file
{
  ...
  */
  ...
  while (getline(file, line)) {
    ...
    if (parts.size() >= 7) {
      int value =
          stoi(parts[0]); // value == 1–5  stoi is str2int
